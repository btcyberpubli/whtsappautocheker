// popup.js - Control UI para WhatsApp Auto Checker - Nueva Interfaz
...
function updateStatus() {
  chrome.storage.local.get(
    ...
    (result) => {
      ...
      const nextRun = result.nextRun || 'Esperando...';
      ...
      if (statusDot) {
        ...
        statusDot.classList.add(isRunning ? 'active' : 'inactive');
      }
      
      // Actualizar colores de texto
      estadoEl.className = isRunning ? 'status-value running' : 'status-value stopped';
      
      scannedEl.textContent = scanned;
      unreadEl.textContent = unread;
      unreadEl.className = unread > 0 ? 'stat-value error' : 'stat-value';
